...
const pokeIdOver99 = pokémon.filter(p=>p.id > 99).map(p=>p.name);
console.log(pokeIdOver99); 

//an array with just the names of the pokémon whose only type is poison
const poisonPokemon = pokémon.filter(p => p.types.length === 1).filter(p => p.types.includes("poison")).map(p => p.name);
...
var normalCount = pokémon.filter(p => p.types.includes("normal")).length;
